...
result_t os_start_timer(UINT16 ticks,
	expiry_function function,
        ...
        es_timer *timer)
{
	LOG_D("os_start_timer()\n\r");
	timer_start(ticks, exp_os_timer, data, timer);
...
{
	eeprom_read(EEPROM_IO_ADDRESS_ADDR, (u8 *) address);
	return (SUCCESS);
}
